...
#ifndef DEVICE_DELEGATE_NREAL_DOT_H
#define DEVICE_DELEGATE_NREAL_DOT_H
...
class DeviceDelegateNreal;
typedef std::shared_ptr<DeviceDelegateNreal> DeviceDelegateNrealPtr;

class DeviceDelegateNreal : public DeviceDelegate {
public:
  static DeviceDelegateNrealPtr Create(vrb::RenderContextPtr& aContext, android_app* aApp);
...
protected:
  struct State;
  DeviceDelegateNreal(State& aState);
  virtual ~DeviceDelegateNreal();
private:
  State& m;
  VRB_NO_DEFAULTS(DeviceDelegateNreal)
};

} // namespace crow

#endif // DEVICE_DELEGATE_NREAL_DOT_H
